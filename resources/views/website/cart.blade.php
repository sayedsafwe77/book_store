                ...
                <div class="col-12">
                    @forelse ($books as $book)
                        <div class="item-cart row">
                            ...
                            @endphp
                            @livewire('add-to-card-component', ['book' => $book, 'quantity' => $quantity])
...
@push('js')
    <script>
        Array.from(document.querySelectorAll('.decrement_quantity')).forEach(sub => sub.addEventListener('click',decrementQuantity));
        Array.from(document.querySelectorAll('.increment_quantity')).forEach(add => add.addEventListener('click',incrementQuantity));
        ...
        function deboumce(time){
            if(debounce_id){
                clearTimeout(debounce_id);
            }
            debounce_id = setTimeout(() => {
                    document.querySelector('#cart_quantity').submit();
                }, time);
        }
    </script>
@endpush --}}
